...
- Testes: `cargo test` (rodar de `aika-rs/`). Hoje: 429 no aika-server, 74 no aika-data, 9 no aika-net, 8+10 integração — **530**. **Manter tudo verde.**
...
- **O campo de skill do registro é uma MÁSCARA DE BITS, não um nível.** Custou a sessão inteira e é a descoberta que vale mais do dia. `GetSkillLevel` (`SkillFunctions.pas:38`) escreve `2^(rank+1) - 2`: um nível é `10`, dois é `110`, três é `1110`. Mandar a contagem crua faz o cliente ler como máscara, achar um rank abaixo do que tem, e **pedir de novo o rank que acabou de comprar — para sempre**. É o que fazia "comprar e não subir".
- **O `$2` das basics nunca foi marcador.** É essa mesma fórmula com um nível. Estava no código desde o começo como "marcador que ninguém sabia explicar".
- **Aritmética do cliente, MEDIDA (não deduzida):** ele pede **`Index + rank`**, com `rank` lido da máscara. Provas do log desta sessão: slot 35 em nível 0 pediu 562 (= o `Index` do slot); slot 6 em nível 3 pediu 102 (= `Index+4`); slot 9 em nível 2 pediu 149 (= `Index+3`).
- **Desvio deliberado do Delphi (o único até hoje neste módulo):** a máscara sai do **nível que guardamos**, não da coluna `Level@12` da tabela. Sob a numeração do próprio original a coluna vale sempre `nível+1` — `GetSkillIndex(classe,slot,1)` cai em `base+slot*16+1`, que a tabela chama de rank 2 (97 é o Ataque Poderoso rank 1 e o registro aponta pro 98). Três razões, todas escritas no doc do `set_player_skills`: (a) o original **já faz essa correção à mão** cravando `2` nas basics em vez de calcular; (b) só assim a aritmética do cliente fecha (rank = nível ⇒ comprar avança um); (c) 16 níveis caem nos 16 ids que o slot tem em vez de o último invadir a skill seguinte. **Se algum dia isso parecer errado, é aqui que se olha.**
- **`T_SkillData` (`FilesData.pas:146`): `Index, MinLevel, Null_3, Level, Classification`.** O `Index@0` **não é o id** — é a família/nome (os 16 ranks do "Ataque Poderoso", ids 97..112, todos têm `Index@0 = 1`). O id é o número do registro. Quase caí nessa.
- **Recusa silenciosa do SERVIDOR também trava o cliente.** O guard `SkillUpgraded` responde nada a um pedido repetido — igualzinho ao original, que nunca precisa porque o cliente dele não repete. O nosso repetiu 9× contra o silêncio e o jogo travou inteiro (não andava, não abria nada). A lição do handoff era sobre o cliente recusar calado; vale nos dois sentidos.
